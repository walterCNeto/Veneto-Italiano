# Curso Vêneto & Italiano — Ciclo Básico 🦁🐺

Aprendizado simultâneo das duas línguas, do som à primeira conversa.
Baseado nas Aulas 1–4 e Fundamentos do curso **Łengua Mare** (Fernando Menegatti).

## Como estudar

1. Cada etapa = uma pasta, na ordem. Leia a `licao.md`, **em voz alta**.
2. Faça as **10 questões de múltipla escolha** da `exercicios.md` **sem consultar a lição**, anotando suas respostas. Só então abra o **gabarito ao final** e some os pontos.
3. Critério para avançar: **mínimo 8/10 (80%)**. Abaixo disso, releia a lição e refaça amanhã.
4. Regra do espelho: tudo que disser numa língua, diga imediatamente na outra.
5. Sessões de 15–20 min. Melhor todo dia pouco do que muito uma vez.

## Trilha

| Etapa | Tema | Status |
|---|---|---|
| [01](etapa-01-sons/licao.md) | Sons e alfabeto | ☐ |
| [02](etapa-02-dialogo-minimo/licao.md) | Diálogo mínimo (saudações) | ☐ |
| [03](etapa-03-cliticos/licao.md) | Clíticos de sujeito | ☐ |
| [04](etapa-04-esar-essere/licao.md) | Verbo ser — èsar / essere | ☐ |
| [05](etapa-05-verbos-ar/licao.md) | Verbos regulares -ar / -are | ☐ |
| [06](etapa-06-interrogativa/licao.md) | Interrogativa vêneta | ☐ |
| [07](etapa-07-primeiras-frases/licao.md) | Primeiras frases + bússola | ☐ |
| [08](etapa-08-dialogos/licao.md) | Diálogos-modelo (prova final) | ☐ |

Marque ☑ ao concluir (edite este README — o commit vira seu registro de progresso).

## Convenções

- 🦁 **Vêneto** · 🐺 **Italiano**
- *Da boca / Colloquiale* = informal (ti/tu) · *Da Gran Siori / Formale* = formal (vu/Lei)
- Grafia vêneta: S sempre surdo, Z sempre sonoro, Ł evanescente, NJ = "nh", sem letras duplas.
